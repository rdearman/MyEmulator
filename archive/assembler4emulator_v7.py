...
class Assembler:
    def __init__(self):
        ...
        self.binary_data = []  # Initialize an empty list to store binary instructions
    ...
    def pre_pass(self, lines):
        ...
        for line in lines:
            ...
            if line.endswith(':'):
                ...
                # Construct the machine code instruction and append it to the list
                binary_instruction = int(''.join(line.split()[1:]), 2)
                self.binary_data.append(binary_instruction)  # Store binary instructions
...
#            operand_list = re.split(r'[,\s]*', immediate_text)
    ...
    def write_binary_output(self, output_file, binary_data):
        with open(output_file, 'wb') as f:
            for binary_instruction in binary_data:
                # Write each binary instruction as bytes
                f.write(binary_instruction.to_bytes(2, byteorder='big'))
...
if __name__ == "__main__":
    ...
    try:
        base_name = os.path.splitext(os.path.basename(input_file))[0]
        output_file = f"{base_name}.bin"  # Define base_name here

        assembler.assemble(input_file)

        # Write the binary data to the output file
        assembler.write_binary_output(output_file, assembler.binary_data)

        print(f"Assembly completed successfully. Binary output written to '{output_file}'.")
