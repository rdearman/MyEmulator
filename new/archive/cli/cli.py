...
class CommandLineInterface:
    ...
    def handle_syscall(self, syscall_number, args):
        ...
        if syscall_number == SYS_PRINT:
            ...
            message_address = args[0]
            message = self.emulator.read_string(message_address)
            print(message)
            return 0  # Success
        elif syscall_number == SYS_EXIT:
            # Execute the exit system call
            # Handle the exit within the CLI, if needed
            return self.exit_cli()  # Terminate the CLI
    ...
    def start(self):
        ...
        while not self.emulator.exit_event.is_set():
            # Check for system call requests in the queue
            try:
                syscall_number, args = self.system_call_queue.get(timeout=1)  # Adjust timeout as needed
                self.handle_syscall(syscall_number, args)
            except queue.Empty:
                pass  # No system call request, continue with regular CLI input processing
